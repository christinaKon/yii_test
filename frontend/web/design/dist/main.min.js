...
return Select2;});return{define:S2.define,require:S2.require};}());var select2=S2.require('jquery.select2');jQuery.fn.select2.amd=S2;return select2;}));$(document).ready(function(){slider={attach:function(context,settings){var opt=[{slider:'.how-it-works .slider',options:{arrows:false,dots:true}},{slider:'.brands .slider',options:{autoplay:true,arrows:false,slidesToShow:5}},{slider:'.clients-reviews .slider',options:{arrows:false,dots:true}}];for(var i=0;i<opt.length;i++){this.initialize(opt[i].slider,opt[i].options);}},initialize:function(slider,options,context){$(slider,context).slick(options);}};select={attach:function(context,settings){$("select").select2({allowClear:true});}};slider.attach();select.attach();});